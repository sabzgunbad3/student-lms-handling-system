...
let authenticatedStudents: string[] = [];
 ...
 */
function isAutomationTask(messageText: string): boolean {
 ...
 */
async function queryLlama3Text(prompt: string): Promise<string> {
  ...
  if (body.object === 'whatsapp_business_account') {
    ...
    if (messageText.startsWith('/unlock')) {
      ...
      if (token === ADMIN_TOKEN) {
        ...
        await sendWhatsAppMessage(
          studentPhone,
          'Heavy Automation Access Granted! Please send your LMS configuration or download request within the next 5 minutes.'
        );
      } else {
        console.log(`[Auth Fail] Invalid unlock token sent by ${studentPhone}: "${token}"`);
        await sendWhatsAppMessage(
          studentPhone,
          'Invalid Access Token. Please provide the correct Admin Secret Key to unlock heavy automation.'
        );
      }
      return;
    }
    ...
    if (isAutomationTask(messageText)) {
      ...
      if (!isAuth) {
        ...
        await sendWhatsAppMessage(
          studentPhone,
          'Access Denied. This heavy automation feature requires an Admin Secret Key. Please contact the Admin to get your access token.'
        );
        return;
      }
      ...
      try {
        ...
        await sendWhatsAppMessage(
          studentPhone,
          'Error forwarding your task to the automation core. Please try again shortly.'
        );
      }
    } else {
...
app.post('/callback/deauth', (req: Request, res: Response) => {
