...
interface Session {
  ...
  studentPhone: string;
  studentName: string;
  status: 'PROCESSING' | 'WAITING_FOR_OTP';
  stepsHistory: string[];
  inactivityTimer: NodeJS.Timeout | null;
  otpSelector?: string;
  otpCoordinates?: { x: number; y: number };
}
 ...
 */
async function deauthorizeOnVercel(studentPhone: string) {
  try {
 ...
 */
async function closeSession(studentPhone: string) {
 ...
 */
function resetInactivityTimer(session: Session) {
  if (session.inactivityTimer) {
    clearTimeout(session.inactivityTimer);
  }
  ...
  session.inactivityTimer = setTimeout(async () => {
    ...
    try {
      await sendWhatsAppMessage(
        ...
        'Your automation session has timed out due to 5 minutes of inactivity. For security, your session has been closed.'
      );
    } catch (e) {
 ...
 */
async function getNextActionFromVision(
  ...
  const systemInstructions =
    `You are a visual web automation assistant. You are looking at a screenshot of a browser viewport sized exactly 1280x800.\n` +
    `Your goal is: "${goal}".\n` +
 ...
 */
async function runVisualAutomationLoop(session: Session, goal: string) {
  let step = 0;
  const maxSteps = 12;
  ...
  while (step < maxSteps) {
    ...
    const screenshot = await session.page.screenshot({ type: 'png' }) as Buffer;
    ...
    if (aiResult.action === 'wait_for_otp') {
      ...
      await sendWhatsAppMessage(
        session.studentPhone,
        `Secure Verification Needed: ${aiResult.reason || 'Please provide your OTP code.'} Reply with the OTP code within 5 minutes.`
      );
 ...
 */
async function resumeWithOTP(session: Session, otpCode: string) {
 ...
 */
async function handleMemorySafeDownloadAndUpload(studentPhone: string, sourceUrl: string) {
  ...
  const tempFilePath = path.join(tempDir, `${fileId}.bin`);
...
app.post('/automation', (req: Request, res: Response) => {
